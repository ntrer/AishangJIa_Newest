...
public class ShouKaFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener, BaseQuickAdapter.RequestLoadMoreListener{
    ...
    private void getData(String activity_id, String merchantId) {
        if(isAll){
             url = BaseUrl.BASE_URL+"customerActionController.do?method=getCustomerActionsByActivityId&token_id="+token_id+"&page=1"+"&rows=10&activityId="+activity_id+"&loginOS=2";
        }
        else {
             url = BaseUrl.BASE_URL+"customerActionController.do?method=getCustomerActionsByActivityId&token_id="+token_id+"&page=1"+"&rows=10&activityId="+activity_id+"&merchantId="+merchantId+"&loginOS=2";
        }
        Log.d("getshouka",url);
        RestClient.builder()
                .url(url)
